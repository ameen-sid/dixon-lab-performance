...
export default function ProductsPage() {
	...
	const handleSave = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!nameInput.trim()) { setError("Product name is required."); return; }
		setSaving(true);
		setError("");
		try {
		...
		<div className="max-w-5xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
			...
			<div className="flex justify-between items-end mb-8">
				<div>
					...
					<p className="text-slate-500 mt-1 font-medium">
						Manage the master list of products and part names across the portal.
					</p>
				</div>
				<button
					onClick={openAdd}
			...
			{loading ? (
				...
				<div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-16 text-center">
					...
					<p className="font-bold text-slate-500">No products yet</p>
					<p className="text-sm text-slate-400 mt-1">Add your first product to populate the dropdown in functional tests.</p>
				</div>
			) : (
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					{products.map((p, idx) => (
						...
						<form onSubmit={handleSave} className="p-8 space-y-5">
							...
							<div>
								...
								<input
									...
									placeholder="e.g. MODIFIED TUB, CLASP AREA"
								/>
							</div>
							<div className="flex justify-end gap-3 pt-2">
								...
								<button type="submit" disabled={saving} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-slate-400 disabled:to-slate-400 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg shadow-blue-500/20 transition-all text-sm flex items-center gap-2">
									{saving ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : (editTarget ? "Update" : "Add Product")}
								</button>
							</div>
						</form>
					</div>
