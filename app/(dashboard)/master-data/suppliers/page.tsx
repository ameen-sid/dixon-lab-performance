...
export default function SuppliersPage() {
	...
	const handleSave = async (e: React.FormEvent) => {
		...
		try {
			...
			if (res.ok) {
				setSuccess(editTarget ? "Entry updated!" : "Entry added!");
				setShowModal(false);
				fetchSuppliers();
	...
	return (
		<div className="max-w-6xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
			...
			<div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
				<div>
					<h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Supplier / Customer Management</h2>
					<p className="text-slate-500 mt-1 font-medium">Manage vendors and client entities associated with testing.</p>
				</div>
					...
					<table className="w-full text-left border-collapse">
						<thead>
							<tr className="bg-slate-50/80 text-slate-500 text-xs uppercase tracking-wider">
								...
								<th className="px-6 py-4 font-bold border-b border-slate-200">Supplier / Customer Name</th>
								<th className="px-6 py-4 font-bold border-b border-slate-200">Registration Date</th>
							...
							{loading ? (
								...
								filteredSuppliers.map((s) => (
									<tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
										<td className="px-6 py-4 font-mono text-sm text-slate-500">#{s.id.toString().padStart(3, '0')}</td>
					...
					<div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto no-scrollbar animate-in fade-in zoom-in-95 duration-200">
						<div className="bg-slate-900 px-8 py-5 flex justify-between items-center rounded-t-3xl sticky top-0 z-10">
							<h3 className="text-white font-bold flex items-center gap-2">
								...
								{editTarget ? "Edit Entity" : "Add New Entity"}
							</h3>
						...
						<form onSubmit={handleSave} className="p-8 space-y-6">
							...
							<div>
								<label className="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">
									Entity Name (Supplier / Customer) <span className="text-red-500">*</span>
								</label>
								<input
									type="text"
									value={nameInput}
									...
									placeholder="e.g. SAMSUNG, DIXON, LG"
								/>
							</div>
							<div className="flex justify-end gap-3 pt-2">
								...
								<button type="submit" disabled={saving} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-slate-400 disabled:to-slate-400 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg shadow-blue-500/20 transition-all text-sm flex items-center gap-2 cursor-pointer">
									{saving ? "Saving..." : (editTarget ? "Update Entry" : "Add Entity")}
								</button>
							</div>
						</form>
					</div>
