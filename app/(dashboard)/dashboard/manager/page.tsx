...
export default async function ManagerDashboard() {
	...
	const activePlansCount = await prisma.testPlan.count({ where: { status: "PLANNED" } });
	const completedPlansCount = await prisma.testPlan.count({ where: { status: "COMPLETED" } });
		...
		<div className="pb-20 animate-in fade-in duration-700">
			...
			<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 shrink-0">
				<div>
					...
					<p className="text-slate-500 font-bold mt-1 text-sm tracking-tight italic">System Status: Optimal | Manager Operations Online</p>
				</div>
			</div>
