...
export default async function DashboardOverview() {
	...
	if (user.role === "Requester") redirect("/dashboard/requester");
	if (user.role === "Head") redirect("/dashboard/head");
	if (user.role === "Lab Manager") redirect("/dashboard/manager");
	if (user.role === "Engineer") redirect("/dashboard/engineer");
		...
		<div className="pb-20 animate-in fade-in duration-700">
			...
			<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 shrink-0">
				<div>
					...
					<p className="text-slate-500 font-bold mt-1 text-sm tracking-tight italic">System Status: Optimal | Global Oversight Online</p>
				</div>
			</div>
